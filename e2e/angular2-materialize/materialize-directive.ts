...
import { dateFromString1, stringFromDate } from '../../../utilities';
...
export class MaterializeDirective implements AfterViewInit, DoCheck, OnChanges, OnDestroy {
  ...
  private performElementUpdates() {
    ...
    if (this.isAutocomplete()) {
      ...
      jQueryElement.on('change',
        e => nativeElement.dispatchEvent((<any>CustomEvent('input'))));
    }
    ...
    if (this.isDatePicker()) {
      ...
      if (nativeElement) {
        const options = _.assign({},
          {
            format: 'dd/mm/yyyy', yearRange: 30, autoClose: true,
            onSelect: (date) => {
              console.log(date);
              nativeElement.value = stringFromDate(date);
              nativeElement.dispatchEvent((<any>CustomEvent('input')));
            }
          },
          this._params);
        const datePicker = M.Datepicker.init(nativeElement, options);
        if (this.ngModel) {
          datePicker.setDate(this.ngModel);
        } else {
          datePicker.setDate(dateFromString1(nativeElement.value));
        }
        this._native = true;
      }
    }
    ...
    if (this.isTimePicker()) {
      ...
      setTimeout(() => {
        ...
        jqueryPickerElement
          .on('change', e => nativeElement.dispatchEvent((<any>CustomEvent('input'))));
      });
    }
    ...
    if (this.isChips()) {
      const nativeElement = this._el.nativeElement;
      const jQueryElement = $(nativeElement);
      jQueryElement.on('chip.add', (e, chip) =>
        nativeElement.dispatchEvent((<any>CustomEvent('chip.add', chip))));
      jQueryElement.on('chip.delete', (e, chip) =>
        nativeElement.dispatchEvent((<any>CustomEvent('chip.delete', chip))));
      jQueryElement.on('chip.select', (e, chip) =>
        nativeElement.dispatchEvent((<any>CustomEvent('chip.select', chip))));
    }
    ...
    if (this.isDropDown()) {
      ...
      if (nativeElement) {
        const params = _.assign({}, this._params, { coverTrigger: false });
        M.Dropdown.init(nativeElement, params);
        this._native = true;
      }
    }
  ...
  private performLocalElementUpdates(functionName = this._functionName, params = this._params) {
    ...
    if (this.isDropDown()) {
      const nativeElement = this._el.nativeElement;
      const instance = M.Dropdown.getInstance(nativeElement);
      switch (functionName) {
        case 'open':
          instance.open();
          break;
  ...
  private isDropDown() {
    return (this._functionName && this._functionName === 'Dropdown');
  }
